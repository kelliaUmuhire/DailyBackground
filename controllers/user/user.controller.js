...
exports.createUser = async (req, res) => {
  try {
    ...
    if (error)
      return res.send(
        formatResult({ status: 400, message: error.details[0].message })
      );
    ...
    return res.send(
      formatResult({ status: 201, message: "CREATED", data: result })
    );
  } catch (e) {
    return res.send(formatResult({ status: 500, message: e }));
  }
...
exports.userLogin = async (req, res) => {
  try {
    ...
    if (!user)
      return res.send(
        formatResult({ status: 404, message: "Invalid credentials" })
      );
...
exports.updateUser = async (req, res) => {
  try {
    ...
    return res.send(
      formatResult({ status: 200, message: "UPDATED", data: result })
    );
  } catch (e) {
    return res.send(formatResult({ status: 500, message: e }));
  }
...
exports.updateUserPassword = async (req, res) => {
  try {
    ...
    if (!validPassword)
      return res.send(
        formatResult({ status: 400, message: "Invalid password" })
      );
    ...
    return res.send(
      formatResult({ status: 201, message: "UPDATED", data: updated })
    );
  } catch (err) {
    return res.send(formatResult({ status: 500, message: err }));
  }
